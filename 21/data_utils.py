...
chords, abstract_grammars = get_musical_data('original_music.mid')
...
def generate_music(inference_model, length, corpus = corpus, abstract_grammars = abstract_grammars, tones = tones, tones_indices = tones_indices, indices_tones = indices_tones, T_y = 10, max_tries = 1000, diversity = 0.5):
    """
    ...
    for i in range(1, num_chords):
        ...
        x_initializer = np.zeros(shape=(1,1,78))
        a_initializer = np.zeros(shape=(1,64))
        c_initializer = np.zeros(shape=(1,64))   
        _, indices = predict_and_sample(inference_model, x_initializer, a_initializer , c_initializer, length)
    ...
    mf.open("generated_music.midi", 'wb')
    mf.write()
    print("Your generated music is saved as generated_music.midi")
    mf.close()
...
def predict_and_sample(inference_model, x_initializer, a_initializer, 
    ...
    for i in range(length):
        z = inference_model.predict([x_initializer, a_initializer, c_initializer])
        idx = np.argmax(z[0])
        indices.append([np.argmax(z[0])])
        x_initializer = np.zeros(shape=(1, 1, 78), dtype='float32')
        x_initializer[0,0,idx] = 1.0
        results.append([x_initializer])
        a_initializer = z[1]
        c_initializer = z[2]
